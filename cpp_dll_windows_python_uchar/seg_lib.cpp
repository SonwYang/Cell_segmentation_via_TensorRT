...
public:
	...
	uchar* prediction(int rows, int cols, unsigned char *src_data) {
		...
		uchar* buffer = (uchar*)malloc(sizeof(uchar)* rows * cols * 3);
		memcpy(buffer, im_color.data, rows * cols * 3);
		return buffer;
...
extern "C" {
	...
	extern "C" _declspec(dllexport) uchar* prediction(int rows, int cols, unsigned char *src_data);
