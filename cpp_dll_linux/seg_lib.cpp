...
public:
	...
	void prediction(int rows, int cols, unsigned char *src_data, unsigned char *ret_data) {
		...
		int count_pixel = 0;
		//std::cout << "start generating out image" << std::endl;
		...
		//std::cout << "generate out image done" << std::endl;
		//delete prob;
		//delete blob;
		...
		memcpy(ret_data, im_color.data, rows*cols * 3);
		im_color.release();
		outimg.release();
...
extern "C" {
	MedicalSeg ms;
	extern "C" void init(char *engine_path) {
		ms.init(engine_path);
	}

	extern "C" void prediction(int rows, int cols, unsigned char *src_data, unsigned char *ret_data) {
		ms.prediction(rows, cols, src_data, ret_data);
	}
}
